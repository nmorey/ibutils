...
proc InitializeIBDIAG {} {
   ...
   set G(var:list.skip.steps) "dup_guids zero_guids pm logical_state"
   set G(var:list.skip.steps) "dup_guids zero_guids pm logical_state load_ibdm"
...
proc DeleteOldFiles {} {
   ...
   foreach fileName $list_filesNames  {
      if {[string range $fileName end-2 end] == "log"} {
         continue
      }
...
proc DiscoverFabric { _pathLimit {startIndex 0}} {
   ...
   while { $index_dr < [llength $G(data:list.direct.path)] } {
      ...
      set bool_skipGuids [BoolWordInList dup_guids $G(argv:skip.checks)]
...
proc DumpBadLidsGuids { args } {
   ...
   set bool_skipZeroGuids [BoolWordInList zero_guids $G(argv:skip.checks)]
   set bool_skipDupGuids [BoolWordInList dup_guids $G(argv:skip.checks)]
...
proc DumpBadLinksLogic {} {
   ...
   if {[BoolWordInList logical_state $G(argv:skip.checks)]} {
      inform "-I-reporting:skip.set.no.report"
      return 1
   }
...
proc PMCounterQuery {} {
   ...
   if {[BoolWordInList pm $G(argv:skip.checks)]} {
      inform "-I-reporting:skip.set.no.report"
      return 1
   }
...
proc GetArgvPortNames {} {
   global G argv
   if { ![info exists G(argv:by-name.route)] || [BoolWordInList load_ibdm $G(argv:skip.checks)]} {
      return
   }
   set flag "-n"
...
proc DumpFabQualities {} {
   global G SM
   if {[info exists G(lst.failed)] || ![info exists G(argv:report)] || [BoolWordInList load_ibdm $G(argv:skip.checks)]} {
      return 1
   }
