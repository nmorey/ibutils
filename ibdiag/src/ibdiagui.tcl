...
#	puts "-V- System:$SYSTEM($sys) LABEL: $sysLabel"
}
...
proc drawFabric {fabric c {sysList 0}} {
   ...
   # we might get a sorted ssytem list
   if {$sysList == 0} {
      set sysList [lsort -index 0 [IBFabric_SystemByName_get $fabric]]
   }

   # we add each system as a subgraph and then
   foreach nameNSys $sysList {
   ...
   foreach nameNNode [IBFabric_NodeByName_get $fabric] {
	...
	SetStatus "Calculating graph layout ..."
   $graph layout NEATO
	SetStatus "Packing graph ..."
...
# 
##############################################################################
proc PropsUpdate {objType ibdmHandle} {
	global P
...
proc LogObjSelect {log type w x y} {
	...
	switch $type {
		NAME {
			if {[guiHighLightByName system $val] != ""} {
				set x 1
			} elseif {[guiHighLightByName node $val] != ""} {
				set x 2
			} elseif {[guiHighLightByName port $val] != ""} {
				set x 3
			} else {
				guiHighLightByName sysport $val
			}
		}
		LID {
		...
		{sys SYSTEM
			{
				...
				type Type "" 
				guid GUID "" 
				nodes "\#Node" {PropsUpdate node $PROPS(sys,node,id)}
			}
		}
		{node NODE 
			{
				name Name "" 
				guid GUID ""
				sys  System     {PropsUpdate system $PROPS(node,sys,id)}
				ports "\#Ports" {PropsUpdate port  $PROPS(node,port,id)}
				dev "Device ID" ""
				rev "Revision ID" ""
				vend "Vendor ID"
			}
		...
		{port PORT
			{
				...
				speed Speed ""
				width Width ""
				node Node    {PropsUpdate node    $PROPS(port,node,id)}
				rem Conn     {PropsUpdate port    $PROPS(port,rem,id)}
				sysp SysPort {PropsUpdate sysport $PROPS(port,sysp,id)}
			}
		}
		{sysport "FRONT PANEL PORT"
			{ 
				name Name ""
				sys System         {PropsUpdate system  $PROPS(sysport,sys,id)}
				port "Node Port"   {PropsUpdate port    $PROPS(sysport,port,id)}
				rem "Connected to" {PropsUpdate sysport $PROPS(sysport,rem,id)}
				width Width ""     
				speed Speed ""
			}
		}
	...
	foreach propSet $props {
		...
		foreach {attr lbl cmd} [lindex $propSet 2] {
			...
			if {[string range $lbl 0 0] == "\#"} {
				...
				pack $f.$attr.m -side right -anchor e
			...
			if {$cmd != ""} {
				bind $f.$attr.l <ButtonPress-1> $cmd
				bind $f.$attr.v <ButtonPress-1> $cmd
			}
		}
...
proc DiagNet {} {
	...
	if {$testModeDir != 0} {
		...
		if {[catch {set r [eval "exec ibdiagnet $argv"]} e]} {
